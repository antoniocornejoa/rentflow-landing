...
/** Botón flotante de WhatsApp (sticky), registra el clic como lead. */
export function WhatsAppFloat({
  whatsapp,
  ctx,
}: {
  whatsapp: TenantContent["whatsapp"];
  ctx: CtaContext;
}) {
  if (whatsapp.mostrar_flotante === false) return null;
  return (
    <div className="fixed bottom-4 right-4 z-50">
      <WhatsAppLink
        tenantId={ctx.tenantId}
        numero={whatsapp.numero}
        mensaje={whatsapp.mensaje_prellenado}
        demo={ctx.demo}
        className="flex items-center gap-2 rounded-full bg-[#25D366] px-5 py-3 font-semibold text-white shadow-lg transition-transform active:scale-95"
      >
