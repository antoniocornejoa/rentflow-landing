...
--
-- PERF: se envuelven is_admin()/current_tenant_ids()/auth.uid() en subselect
-- escalar `(select ...)` para que el planner los evalúe UNA vez por consulta
-- (InitPlan), no por fila (patrón recomendado por Supabase: auth_rls_initplan).
-- ============================================================================
...
create policy tenants_select_own on public.tenants
  ...
  using ((select public.is_admin()) or id in (select public.current_tenant_ids()));
...
create policy tenant_domains_select_own on public.tenant_domains
  for select to authenticated
  using ((select public.is_admin()) or tenant_id in (select public.current_tenant_ids()));
...
create policy leads_update_own on public.leads
  ...
  using ((select public.is_admin()) or tenant_id in (select public.current_tenant_ids()))
  with check ((select public.is_admin()) or tenant_id in (select public.current_tenant_ids()));
